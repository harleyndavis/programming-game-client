...
| `src/memory.ts` | Persistent SQLite-backed (`better-sqlite3`) world knowledge store (tracked in #6) — `openMemoryDb`, plus record/get functions for safe locations, merchant knowledge, explored cells, world heat map (monster/NPC/resource/station sightings), combat history, drop tables, and quests. A dumb store: records what it's told, no domain judgement. Write/read functions take and return game-native SDK types (`Position`, `Items`, `Monsters`, `ClientSideNPC`, `ClientSideMonster`, `GameObject`, `ActiveQuest`) or thin wrappers around them — never bespoke re-derived shapes. Not yet wired into `index.ts` — see ADR-0003's "create the file, don't touch `index.ts`" sequencing. |
