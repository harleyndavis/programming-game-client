...
| `src/memory.ts` | Persistent SQLite-backed (`better-sqlite3`) world knowledge store (tracked in #6) — `openMemoryDb`, plus record/get functions for safe locations, an `entities` catalog, merchant knowledge, explored cells, world heat map, combat history, drop tables, and quests. The `entities` table is the canonical registry of every distinct entity type/species/subtype ever seen (`getEntity`, `getKnownEntities`) — heat map, merchants, combat history, drop tables, and quests all reference an entity by surrogate `id` rather than duplicating the type/name pair; heat_map stays purely spatial ("where/when"), entities answers "have we ever seen this at all". A dumb store: records what it's told, no domain judgement. Write/read functions take and return game-native SDK types (`Position`, `Items`, `Monsters`, `ClientSideNPC`, `ClientSideMonster`, `GameObject`, `ActiveQuest`) or thin wrappers around them — never bespoke re-derived shapes. Not yet wired into `index.ts` — see ADR-0003's "create the file, don't touch `index.ts`" sequencing. |
