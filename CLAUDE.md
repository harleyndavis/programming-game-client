...
| `src/memory.ts` | Persistent SQLite-backed (`better-sqlite3`) world knowledge store (tracked in #6) — `openMemoryDb`, plus record/get functions for safe locations, an `entities` catalog, merchant knowledge, explored cells, world heat map, combat history, drop tables, and quests. The `entities` table is the canonical registry of every distinct entity ever seen (pure identity — `id`, `entity_type`, `entity_name`, no timestamp; `getEntity`, `getKnownEntities`) — heat map, combat history, drop tables, merchants, and quests all reference an entity by surrogate `id` rather than duplicating the type/name pair; each of those tables tracks its own "when" for its own context, so the catalog doesn't. What `entity_name` holds depends on whether individuals of that type are unique or interchangeable: monsters and resources are interchangeable instances of a species/type (`monster.monsterId`, `treeType`/`oreType` — many rats or pine trees collapse into one `entities` row each), but NPCs are individually named, persistent characters (`npc.name`, e.g. "Aigor the Merchant" — never `npcType`, which is a shared role, not an identity: every merchant would otherwise collapse into one indistinguishable entity). `merchants` and `quests` are per-individual-NPC detail tables that reference their NPC's own entity, the same way a future per-type detail table (e.g. harvest yields for tree/ore entity types) would reference its type's entity. A dumb store: records what it's told, no domain judgement. Write/read functions take and return game-native SDK types (`Position`, `Items`, `Monsters`, `ClientSideNPC`, `ClientSideMonster`, `GameObject`, `ActiveQuest`) or thin wrappers around them — never bespoke re-derived shapes. Not yet wired into `index.ts` — see ADR-0003's "create the file, don't touch `index.ts`" sequencing. |
